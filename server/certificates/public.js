...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!cert || !recordAllowedForEnvironment(cert) || !['generated', 'sent'].includes(cert.status)) return sendJson(res, 404, { ok: false, error: 'CERTIFICATE_NOT_FOUND' });
  ...
  } catch (error) { return sendJson(res, 500, { ok: false, error: error.code || 'CERTIFICATE_READ_FAILED' }); }
};
