...
function certificateIdForSession(sessionId) {
...
async function fulfilPaidSession(session, req) {
  ...
  if (String(session.currency || '').toLowerCase() !== 'eur' || Number(session.amount_total || 0) !== tier.amount * 100) {
    throw Object.assign(new Error('Stripe amount does not match canonical sponsorship tier'), { code: 'STRIPE_AMOUNT_MISMATCH' });
  }
  ...
  const certId = certificateIdForSession(session.id);
  ...
  const order = {
    ...
    source: 'stripe-test-checkout',
    createdAt: now,
    paidAt: now,
  };
  const certificate = {
    sponsorshipId: session.id,
    orderNumber,
  ...
  try {
    await commitWrites([
      updateFieldsWrite('clubs', club.id, { sponsoredPlaces: sponsored + tier.children, updatedAt: now }, club._updateTime),
      createWrite('sponsorships', session.id, order),
      createWrite('certificates', certId, certificate),
    ]);
  } catch (error) {
    const after = await getDocument('sponsorships', session.id).catch(() => null);
    if (after) return { idempotent: true, order: after };
    throw error;
  }
  ...
  try {
    ...
    if (delivery.ok) {
      await updateDocument('certificates', certId, { status: 'sent', deliveryStatus: 'sent', sentAt: new Date().toISOString(), updatedAt: new Date().toISOString(), emailProviderId: delivery.id });
      await updateDocument('sponsorships', session.id, { certificateStatus: 'sent' });
      await createDocument('operations_events', { type: 'certificate.sent', certificateId: certId, sponsorshipId: session.id, createdAt: new Date().toISOString() });
    } else {
      await updateDocument('certificates', certId, { deliveryStatus: 'pending_configuration', updatedAt: new Date().toISOString() });
    }
  } catch (error) {
    await updateDocument('certificates', certId, { status: 'failed', deliveryStatus: 'failed', lastError: error.code || 'EMAIL_DELIVERY_FAILED', updatedAt: new Date().toISOString() }).catch(() => null);
    await updateDocument('sponsorships', session.id, { certificateStatus: 'failed' }).catch(() => null);
  }
...
async function handler(req, res) {
  ...
  try {
    ...
    if (relevant && session?.object === 'checkout.session' && session.payment_status === 'paid') await fulfilPaidSession(session, req);
    return sendJson(res, 200, { received: true });
  } catch (error) {
    return sendJson(res, 500, { ok: false, error: error.code || 'STRIPE_WEBHOOK_FAILED' });
  }
}
