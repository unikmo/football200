...
const { writeAllowed, releaseState } = require('../_lib/release');
const { productIdentity } = require('../_lib/idempotency');

module.exports = async function handler(req,res){
  if(req.method!=='POST') return sendJson(res,405,{ok:false,error:'METHOD_NOT_ALLOWED'});
  if(!writeAllowed('minor-payment')) return sendJson(res,403,{ok:false,error:'RELEASE_GATE_BLOCKED'});
  ...
  try{
    ...
    const confirmation=confirmations.find(item=>(item.syntheticTest===true)===expectSynthetic&&item.passReference===passReference&&String(item.guardianEmail||'').toLowerCase()===guardianEmail);
    if(!confirmation)return sendJson(res,409,{ok:false,error:'CONFIRMED_PASS_REQUIRED'});
    const alreadyActive=orders.find(item=>item.paymentStatus==='paid'&&item.passReference===passReference);
    if(alreadyActive)return sendJson(res,409,{ok:false,error:'FAMILY_PLUS_ALREADY_ACTIVE'});
    const identity=productIdentity('family-plus',[passReference,guardianEmail]);const intentId=identity.intentId;const base=deploymentOrigin(req);
    ...
    const session=await stripePost('/checkout/sessions',params,identity.stripeKey);await ensureStripeSessionMode(session);
    return sendJson(res,201,{ok:true,url:session.url,sessionId:session.id,intentId});
