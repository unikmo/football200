...
module.exports = async function handler(req,res){
  ...
  try{
    ...
    const confirmation=(await listAllDocuments('guardian_confirmations')).find(item=>(item.syntheticTest===true)===expectSynthetic&&item.passReference===passReference&&String(item.guardianEmail||'').toLowerCase()===guardianEmail);
    if(!confirmation)return sendJson(res,409,{ok:false,error:'CONFIRMED_PASS_REQUIRED'});
    const identity=checkoutIdentity('family-plus',body.checkoutRequestId,[passReference,guardianEmail]);const intentId=identity.intentId;const base=deploymentOrigin(req);
    ...
    return sendJson(res,201,{ok:true,url:session.url,sessionId:session.id,requestId:identity.requestId});
