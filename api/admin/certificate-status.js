  ...
  try {
    ...
    if (!canTransitionCertificate(currentStatus, nextStatus)) {
      return sendJson(res, 409, {
        ok: false,
        error: 'INVALID_CERTIFICATE_TRANSITION',
        currentStatus,
    ...
    if (nextStatus === 'failed') {
      update.failedAt = now;
      update.lastError = note || 'Manual failure status set from admin preview';
    }
    if (nextStatus === 'pending' && currentStatus === 'failed') {
      update.retryRequestedAt = now;
    ...
    if (orderId) {
      const order = await getDocument('sponsorships', orderId);
      if (order) await updateDocument('sponsorships', orderId, { certificateStatus: nextStatus, updatedAt: now });
    }

    await createDocument('operations_events', {
      ...
      toStatus: nextStatus,
      note,
      source: 'football200-admin-preview',
      createdAt: now,
    });
    ...
    return sendJson(res, 500, {
      ok: false,
      error: error.code || 'CERTIFICATE_STATUS_UPDATE_FAILED',
