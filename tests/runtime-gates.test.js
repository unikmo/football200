...
test('production public and payment writes stay blocked until explicit gates are enabled', () => withEnv({ NODE_ENV:'production', VERCEL_ENV:'production', PUBLIC_RELEASE_ENABLED:'false', LEGAL_RELEASE_APPROVED:'false', PAYMENTS_RELEASE_APPROVED:'false' }, () => {
  assert.equal(writeAllowed('public'), false);
  assert.equal(writeAllowed('payment'), false);
  assert.equal(expectedStripeLivemode(), true);
}));

test('production payment writes require public, legal and payment approval together', () => withEnv({ NODE_ENV:'production', VERCEL_ENV:'production', PUBLIC_RELEASE_ENABLED:'true', LEGAL_RELEASE_APPROVED:'true', PAYMENTS_RELEASE_APPROVED:'true', STRIPE_EXPECTED_ACCOUNT_ID:'acct_live_expected' }, () => {
...
test('minor-payment remains separately gated', () => withEnv({ NODE_ENV:'production', VERCEL_ENV:'production', PUBLIC_RELEASE_ENABLED:'true', LEGAL_RELEASE_APPROVED:'true', PAYMENTS_RELEASE_APPROVED:'true', MINOR_DATA_RELEASE_APPROVED:'false' }, () => {
  assert.equal(writeAllowed('payment'), true);
