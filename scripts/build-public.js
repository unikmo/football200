...
function addNetworkAnalytics(html){if(!analyticsNetworkEnabled||html.includes('data-f200-gtm'))return html;const tag=`<script data-f200-gtm>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],j=d.createElement(s);j.async=true;j.src='https://www.googletagmanager.com/gtm.js?id='+encodeURIComponent(i);f.parentNode.insertBefore(j,f)})(window,document,'script','dataLayer','${gtmId}');</script>`;return html.replace(/<\/head>/i,`${tag}</head>`)}
function productionCopy(relative,html){if(!productionRuntime)return html;if(relative==='sponsor/checkout.html'&&paymentsApproved){html=html.replace('Stripe Sandbox Checkout · Preview only','Sicher bezahlen · Stripe');html=html.replace('Preview/Sandbox. Football200 speichert keine Kartendaten.','Football200 speichert keine Kartendaten.')}if(relative==='family-plus.html'&&paymentsApproved&&minorDataApproved){html=html.replace('Family Plus · Preview','Family Plus · Sicher bezahlen');html=html.replace('Im aktuellen Preview dient der Pass-Referenzwert ausschließlich dem Test des Zahlungs- und Fulfilment-Flows.','Der Pass-Referenzwert bestätigt die Berechtigung für Family Plus.')}return html}

for(const file of walk(root).filter(file=>file.endsWith('.html'))){const relative=path.relative(root,file).replace(/\\/g,'/');let html=fs.readFileSync(file,'utf8');const route=publicFiles.get(relative);if(canIndex&&route){html=setRobots(html,'index,follow,max-image-preview:large,max-snippet:-1,max-video-preview:-1');html=upsertCanonical(html,`${base}${route}`);html=addStructuredData(html,route);html=html.replace(/<div class="demo-bar">Live-Demo · nicht veröffentlicht<\/div>/g,'')}else{html=setRobots(html,'noindex,nofollow,noarchive')}if(route){html=addLocalAnalytics(html);html=addNetworkAnalytics(html)}html=productionCopy(relative,html);fs.writeFileSync(file,html)}
...
console.log(JSON.stringify({env,releaseEnabled,indexingEnabled,legalApproved,paymentsApproved,minorDataApproved,canIndex,analyticsApproved,analyticsConsentReady,analyticsNetworkEnabled,publicPages:publicRoutes.length}));
